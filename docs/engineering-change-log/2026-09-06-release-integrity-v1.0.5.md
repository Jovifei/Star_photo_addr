...
- 火烧云：仍是启发式条件指数映射，用户界面统一使用 `0–100` 条件指数分段，不再把分段显示成事件概率或带 `%` 的概率区间；内部 `probabilityLabel/probabilityLevel` 字段名暂为旧快照兼容保留。修复 p80/p88/p95/p100 高档位统计。
...
1. **WebKit 移动端 dialog 首焦点偶发被触发按钮抢回**：已修复，随后 cross-browser smoke 通过。
2. **旧“4 断点 × 3 路由”横向溢出 E2E 首轮在 60 秒总测试时限触发 timeout**：日志没有出现横向溢出断言失败；保持原断言不降级，已对失败 job 单独重跑。若在最新 HEAD 上仍只表现为整段测试超时，则只为该 12 次导航矩阵用例设置局部更长超时，不放宽任何布局断言。
