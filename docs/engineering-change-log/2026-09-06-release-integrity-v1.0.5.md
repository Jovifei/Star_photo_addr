...
> 历史状态说明（2026-09-16）：本文中的“云海 surface + RH 启发式 Beta”只描述 v1.0.5 当时的实现。后续版本已接入 pressure-level profile、层位关系和逆温证据；当前状态以 v1.0.18 主线和 2026-09-15 模型能力恢复记录为准。
...
- 云海（v1.0.5 当时）：不再生成任何人工天气；关键字段不完整即 `score=null`；相对湿度直接使用 Open-Meteo；云底/云顶仍是启发式估算，页面标记 Beta；显示为“条件指数”，不是现场校准概率。后续版本已接入 pressure-level profile。
...
4. **移除逆温/气压层误导（v1.0.5 历史）**：当时云海仍是 surface-cloud + RH Beta 启发式，因此删除加载态中的“逆温层/气压层高度”声明；后续版本已重新接入真实 pressure-level profile，并保留模式推导边界。
