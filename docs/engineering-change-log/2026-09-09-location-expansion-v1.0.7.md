...
- CI 稳定性：多视口导航总时限从 120 秒提高到 240 秒，首帧卫星/云图条件等待从 15 秒提高到 30 秒；只放宽慢环境等待预算，不改变断言。
- 云图刷新门禁：`map-layer-refresh` 初始 canvas 等待同步为 30 秒，避免慢 runner 在真实画布挂载前误判失败。
- 地图点位门禁：首屏可视 marker 条件等待同步为 30 秒，覆盖 252 点位慢 runner 渲染时间，不放宽可视断言。
