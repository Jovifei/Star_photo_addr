...
> 生产部署源：`main@1e550cdc5a15c87fcad3465fecad9e9bfb3da29a`
...
上一版 v1.0.11 已验证：`npm run check` 的 51 个 Vitest 文件 / 302 项测试通过；Chromium E2E 140 个实例中 106 passed、34 skipped、0 failed。v1.0.12 已验证 `npm run check`（52 个 Vitest 文件 / 304 项测试）、Chromium E2E 144 个实例中 110 passed、34 skipped、0 failed、真实数据源冒烟通过和生产依赖 0 vulnerabilities；生产 app/worker healthy，公网 `/healthz` 与 `check:data-sources` 均返回 v1.0.12 / `1e550cdc5a15`。skipped、MANUAL、BLOCKED 不得写成 PASS。
