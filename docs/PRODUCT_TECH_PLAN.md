...
> 当前实现注记（2026-09-10）：通用观星目录已按省级公开资料扩展到 282 个候选，首页默认精选 17 个，视野候选上限 20 个；本文早期章节中的“12 个预置点”是方案基线，实际交付以 `src/data/observingSites/catalog.json`、`src/lib/constants.ts` 和最新工程修改记录为准。

> 当前完成度注记（2026-09-18）：今夜观测、暗夜选址、火烧云和高山云海四个入口已经实现并进入正式导航。本文的“开发阶段”“Phase 2”章节是路线/研究计划，不是当前页面未完成声明；CloudSea Phase 2 仅指周边谷地采样与科学校准扩展。移动端响应式改进目前仍是等待代码授权的计划。当前模型恢复候选和验证状态见 `docs/GUIDE.md` 与最新工程记录。
