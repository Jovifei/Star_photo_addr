...
> 当前状态（2026-09-18）：四个正式工作区均已实现；火烧云/云海不是待开发入口。云海第一阶段的 surface + pressure profile 已有回归覆盖，CloudSea Phase 2、现场样本/30–90 天准确率校准、真机和性能仍是后续验证项。移动端响应式布局目前只有 `PLAN_READY_WAITING_CODE_AUTHORIZATION` 计划，不能写成已实现。最新候选门禁结果见 [`testing/TEST_STATUS.md`](./testing/TEST_STATUS.md)，不要用本文早期 P4 路线替代当前状态。
