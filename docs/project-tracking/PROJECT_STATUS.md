...
> 状态日期：2026-09-10
> 当前主干代码发布：`main@1e550cdc5a15c87fcad3465fecad9e9bfb3da29a`（v1.0.12）
> 当前活动工作分支：无（`codex/unified-score-filters-20260910` 已合并）
> 当前阶段：v1.0.12 已完成实现、本地质量门禁、main 合并、生产部署和公网验收；真机、性能和科学校准仍按边界管理

## 0. 当前发布快照（v1.0.12 已部署）
...
- 当前主线已通过 `npm run check`（52 个测试文件 / 304 项）、Chromium E2E 110 passed / 34 skipped、live smoke 和 audit；生产 app/worker healthy，公网 `/healthz` 返回 v1.0.12 / `1e550cdc5a15`，`check:data-sources` 通过。
...
| --- | --- | --- | --- | --- |
| — | 无活动代码工作包 | — | v1.0.12 已合并、部署并在公网验证；后续工作按第 5 节 MANUAL/BLOCKED/DEFERRED 管理 | 新需求确认后从最新 `main` 创建唯一 `codex/` 分支 |
