...
> 状态日期：2026-09-10
> 当前主干：`main@d64009276db0dbc55cc1963afbc31587703fbc18`（v1.0.12）
> 当前活动工作分支：无（`codex/unified-score-filters-20260910` 已合并）
> 当前阶段：v1.0.12 已完成实现、本地质量门禁和 main 合并，待生产部署与公网验收；生产仍为 v1.0.11

## 0. 当前发布快照（v1.0.12 已合并，待部署）
...
- 当前主线已通过 `npm run check`（52 个测试文件 / 304 项）、Chromium E2E 110 passed / 34 skipped、live smoke 和 audit；生产仍保持 v1.0.11，待部署后回写公网 SHA。
...
- 本轮新增单元和 Chromium E2E 用例；分支已推送并合并到 main，待部署和公网验收完成前不把 v1.0.12 写成生产 PASS；
...
| 工作包 | 状态 | 交付 | 关键证据 |
| --- | --- | --- | --- |
| RELEASE-UI-012 | READY_TO_DEPLOY | 顶部控件统一、主页默认图层纠偏、火烧云/云海评分门槛滑块、v1.0.12 记录 | `main@d64009276db0`；`npm run check` 52/304、Chromium E2E 110/34、live smoke/audit 通过；待生产部署和公网验收 |
...
| --- | --- | --- | --- | --- |
| RELEASE-UI-012 | READY_TO_DEPLOY | — | v1.0.12 已合并到 `main@d64009276db0`；生产仍为 v1.0.11 | 生产部署、app/worker 健康检查和公网验收；本地完整 E2E/live/audit 已通过 |
