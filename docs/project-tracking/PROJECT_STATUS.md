...
> 状态日期：2026-09-10
> 当前主干代码发布：`main@1f61690e0cba46c59ff1afd79a3562819108508f`（v1.0.12）
> 当前活动工作分支：`codex/search-tai-zi-jian-20260910`（v1.0.13 发布候选）
> 当前阶段：v1.0.13 已完成实现、本地质量门禁和搜索浏览器验收，待推送、合并、部署与公网验收；生产仍为 v1.0.12

## 0. 当前发布快照（v1.0.13 发布候选）
...
- 当前候选已通过 `npm run check`（53 个测试文件 / 306 项）、Chromium E2E 112 passed / 34 skipped、live smoke 和 audit；生产仍为 v1.0.12，待 v1.0.13 发布验收。
...
| 工作包 | 状态 | 交付 | 关键证据 |
| --- | --- | --- | --- |
| RELEASE-UI-013 | IN_PROGRESS | 本地点位搜索优先命中目录、v1.0.13 版本记录和回归测试 | `codex/search-tai-zi-jian-20260910`；`npm run check` 53/306、Chromium E2E 112/34、live smoke/audit 通过；待发布 |
...
| --- | --- | --- | --- | --- |
| RELEASE-UI-013 | IN_PROGRESS | `codex/search-tai-zi-jian-20260910` | 搜索修复与本地门禁已完成；生产仍为 v1.0.12 | 分支 push、main 合并、部署、生产 `/healthz`、公网搜索验收 |
