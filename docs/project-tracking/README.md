...
> 当前活动工作流：模型恢复候选 `codex/model-capability-recovery-20260915`（未合并/未部署）；响应式布局计划 `codex/mobile-responsive-layout-20260918`（仅计划、等待代码授权）

当前四个正式工作区（今夜观测、暗夜选址、火烧云、云海）均已实现。云海第一阶段已经完成；Beta 只表示模式推导和准确率尚未校准，CloudSea Phase 2 的周边谷地采样才是后续扩展。
