...
export default function CloudCanvasOverlay({
  ...
  useEffect(() => {
    ...
    const layers: Array<{ enabled: boolean; values: number[] }> = [
      { enabled: lowEnabled, values: values.low },
      { enabled: midEnabled, values: values.mid },
      { enabled: highEnabled, values: values.high },
    ];
    ...
    for (const layer of layers) {
      ...
      for (let py = 0; py < canvas.height; py += STEP) {
        for (let px = 0; px < canvas.width; px += STEP) {
          ...
          const color = cloudValueToColor(value, 0.55);
          ctx.fillStyle = color;
