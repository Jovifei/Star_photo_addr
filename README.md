...
| 工作区 | 路由 | 主要问题 |
| --- | --- | --- |
| **今夜观测** | `/` | 今晚云量如何、卫星云图怎样、候选地点 7 天排行、地点详情与逐小时轨迹？ |
| **暗夜选址** | `/sites` | 哪些地点远离城市夜光、暗夜星空条件更佳、高程剖面与最佳机位研判？ |
| **火烧云预报** | `/fireglow` | 日落/日出时段晚霞火烧云概率多大、红霞波及范围与最佳视口？ |
| **高山云海** | `/cloudsea` | 名山高程上是否有翻腾云海、逆温层位是否在云上海拔之上？ |

原「观星计划」的深度能力（逐小时走势图、日月银河轨迹图、高山低云垂直剖面分析）已全景整合至「今夜观测」与「暗夜选址」的右侧**地点详情舱**中，形成无缝的一体化观测决策流：

```mermaid
flowchart LR
    A[今夜观测<br/>7天动态排行] --> B[暗夜选址<br/>光污染与天顶亮度]
    B --> C[地点详情舱<br/>气象走势/轨迹/垂直云层]
    C --> D[火烧云 & 云海<br/>特殊天象扩展]
```
...
- 当前选择地点、高程词典与海拔推算；
- 卫星云观测、数值云量预报、光污染参考图层；
- 总云量、高云、中云、低云与小时级气象预报；
- 未来 72 小时预报时次轴与 NASA GIBS 卫星时次；
- 当前数据源状态、抗限流快照装甲与刷新入口；
- **候选地点 · 7 天分数动态排行榜**（名次徽标、气象指标、7 天微缩胶囊走势）；
- **地点详情舱**（最佳连续观测窗口、月相照度、暗夜时长、银河中心高度、逐小时气象矩阵、一键加入/移出候选）。
