...
describe("观星地点查询快照与评分", () => {
  it("包含目标站 252 个地点，Bortle 3 默认显示 228 个", () => {
  ...
  it("跨午夜分析保留空值语义", () => {
    ...
    const analysis = analyzeFinderNightWeather(
      "2026-08-09",
      hourly,
      "photo",
    );
  ...
  it("完整晴空数据可评为完美，缺失数据不会被当成晴空", () => {
    const clear = analyzeFinderNightWeather(
      "2026-08-09",
      fixture(),
      "photo",
    );
  ...
  it("地点详情评分与地点风险共用同一条数据链", () => {
    const location = FINDER_LOCATIONS.find(
      (item) => item.name === "阿里暗夜公园",
    );
  ...
  it("上海日期函数使用运行时日期而不是旧固定日期", () => {
    expect(getShanghaiDate(new Date("2026-08-08T16:30:00.000Z"))).toBe(
      "2026-08-09",
    );
    expect(addFinderDays("2026-08-09", 4)).toBe("2026-08-13");
  });

  it("共享评分保留空值、天气门禁并生成多夜快照", () => {
  ...
  it("uses the exact forecast hour for map distribution counts", () => {
    ...
    const cloudy = fixture({
      cloud_cover: Array.from({ length: 33 }, () => 95),
    });
    ...
    expect(snapshotScoreAtTime(snapshot, site.id)?.score).toBe(
      clearScore.score,
    );
  });

  it("never joins non-consecutive clear hours into a fake 3h window", () => {
    expect(
      findBestContiguousWindow([
        { time: "2026-08-09T20:00", cloud: 5 },
        { time: "2026-08-09T22:00", cloud: 4 },
        { time: "2026-08-09T23:00", cloud: 3 },
      ]),
    ).toEqual([]);
    expect(
      findBestContiguousWindow([
        { time: "2026-08-09T20:00", cloud: 5 },
        { time: "2026-08-09T21:00", cloud: 4 },
        { time: "2026-08-09T22:00", cloud: 3 },
      ]),
    ).toEqual([
      "2026-08-09T20:00",
      "2026-08-09T21:00",
  ...
  it("uses start_date/end_date and enforces each model horizon", () => {
    const url = new URL(
      buildFinderWeatherUrl(
        FINDER_LOCATIONS.slice(0, 2),
    ...
    expect(
      isFinderRangeAllowedForModel(
        Array.from({ length: 7 }, (_, index) =>
          addFinderDays("2026-08-09", index),
        ),
        "icon",
        "2026-08-09",
      ),
