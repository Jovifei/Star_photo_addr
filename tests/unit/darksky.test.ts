...
describe("classifyBortle — 等级阈值", () => {
  ...
  // 回归：nodata 曾被兜底成 B9，等同于伪造「城市中心天空」这一可信读数。
  it("null（nodata）→ null，不冒充 B9", () => {
    expect(classifyBortle(null)).toBeNull();
  });
});

describe("sampleBortle — 缺失资源降级", () => {
  // 默认未安装本地栅格（NEXT_PUBLIC_ASSET_VIIRS_TILES 未开启）。
  it("无本地图层时返回 layer-unavailable，且不含任何等级/数值", async () => {
    const sample = await sampleBortle(30.2741, 120.1551);
    expect(sample.status).toBe("layer-unavailable");
    expect(sample.mpsas).toBeNull();
    expect(sample.bortle).toBeNull();
    expect(sample.bortleName).toBeNull();
    expect(sample.uncertain).toBe(true);
    expect(sample.source).toBe("none");
  });

  it("中国境外同样不伪造读数", async () => {
    const sample = await sampleBortle(48.8566, 2.3522);
    expect(sample.bortle).toBeNull();
    expect(sample.mpsas).toBeNull();
    expect(sample.uncertain).toBe(true);
  });

  it("坐标原样回传，且不抛异常", async () => {
    const sample = await sampleBortle(-33.8688, 151.2093);
    expect(sample.latitude).toBe(-33.8688);
    expect(sample.longitude).toBe(151.2093);
  });
  ...
  it("每种状态都有可展示的中文说明", () => {
    for (const status of [
      ...
      expect(describeDarkSkyStatus(status).length).toBeGreaterThan(0);
    }
  });
});
