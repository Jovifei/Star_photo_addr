...
test("API failure shows error banner and preserves prior data", async ({ page }) => {
  await page.goto("/");
  await expect(page.locator(".rank-card").first()).toBeVisible({ timeout: 20000 });
  // Remove the beforeEach mock so the abort handler is the only one for
  // /api/forecast. Playwright tries routes in registration order, so the
  // earlier mock would otherwise fulfill the request instead of aborting.
